<script lang="ts">
    ...
    function handleMarkAsArrived() {
        ...
        router.patch(
            `/api/flood/mark-as-arrived/${floodAidId}`,
            undefined,
            requestOptions,
        );
    }
...
<Dialog.Root {open} onOpenChange={() => setOpen()}>
    <Button onclick={() => setOpen('open')} size="lg"
        >Tandai bantuan telah sampai</Button
    >
    <Dialog.Content class="w-sm z-50">
        <Dialog.Header>
            <Dialog.Title>Tandai bantuan telah sampai</Dialog.Title>
            <Dialog.Description>
                Pastikan bantuan telah sampai ke lokasi bencana banjir sesuai
                laporan.
            </Dialog.Description>
        </Dialog.Header>
        <Dialog.Footer>
